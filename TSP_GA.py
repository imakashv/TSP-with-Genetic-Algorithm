# -*- coding: utf-8 -*-
"""BIO PROJECT SEM3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qWcu2MvWa_2Ww748SL_faCyJ2f9bcrMx
"""
...
#Method called in the generateFirstPopulation() to  generate a new possible path for the population
def generatePossiblePath():
    path = []
    for _ in range(1, CITIES_SIZE + 1):
        # generates a new number between 1 - 10
...
#Generates the X and Y arrays which represents the distances in the x and y axis used to calculate the identity matrix in the fitness function
def generateXandY():
...
#Generates the Tour matrix, which is the same matrix as the population,but with the first column duplicated at the end of it, afterall, the traveller always have to arrive at the same place of where he started
def generateTour():
    global tour
...
#Generate the identity matrix (dCidade) based on the x and y arrays and then call the calculateDistances() method to generate the array with the sum of each path to user later in the cycle process
def fitnessFunction():
    for i in range(len(population)):
...
#Performs the roulette function, generating two arrays with 5 parents each,which will be used later to do the cycle process
...
def createParents(rouletteArr):
    parentArr = []
    for _ in range(5):
        parentArr.append(rouletteArr[random.randint(0, 10)])
    return parentArr
